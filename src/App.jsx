...
const ALLY_SEED = {
  ...
  unlocks: {
    chains: [
      { id: uid(), name: "Discovery cashbacks", nodes: [
        { id: uid(), label: "Vitality Health Check", sub: "Booked for 3 July", status: "active" },
        { id: uid(), label: "Vitality Age assessment", sub: "Needs the numbers from the health check", status: "locked" },
        { id: uid(), label: "Base HealthyFood live", sub: "Up to 25% back, min R150/mo qualifying spend", status: "locked" },
        { id: uid(), label: "Credit card resubmission", sub: "Reupload statements with the descriptor note. Runs in parallel", status: "active" },
        { id: uid(), label: "Card approved", sub: "Unlocks the boost tier", status: "locked" },
        { id: uid(), label: "Food boost and fuel live", sub: "Up to 75% food, up to 20% fuel at bp / select Shell (min R75/fill)", status: "locked" },
      ] },
...
export default function AllyCRM() {
  ...
  const badges = {
    ...
    unlocks: ally.unlocks.chains.flatMap((c) => c.nodes).filter((n) => n.status === "done").length,
...
function Overview({ ally, mama, go, aptUnlocked }) {
  ...
  const unlockPct = allNodes.length ? allNodes.filter((n) => n.status === "done").length / allNodes.length : 0;
  ...
  if (activeUnlock) schedule.push({ emoji: "🔓", title: activeUnlock.label, sub: activeUnlock.sub || "in progress", tint: C.yellow });
...
function UnlocksModule({ data, setData }) {
  const patchChain = (cid, p) => setData((d) => ({ ...d, unlocks: { chains: d.unlocks.chains.map((c) => (c.id === cid ? { ...c, ...p } : c)) } }));
  const updateNode = (cid, nid, p) => setData((d) => ({ ...d, unlocks: { chains: d.unlocks.chains.map((c) => c.id === cid ? { ...c, nodes: c.nodes.map((n) => (n.id === nid ? { ...n, ...p } : n)) } : c) } }));
  const cycle = { locked: "active", active: "done", done: "locked" };
  const addNode = (cid) => setData((d) => ({ ...d, unlocks: { chains: d.unlocks.chains.map((c) => c.id === cid ? { ...c, nodes: [...c.nodes, { id: uid(), label: "New step", sub: "", status: "locked" }] } : c) } }));
  const removeNode = (cid, nid) => setData((d) => ({ ...d, unlocks: { chains: d.unlocks.chains.map((c) => c.id === cid ? { ...c, nodes: c.nodes.filter((n) => n.id !== nid) } : c) } }));
  return (
    <section>
      <SectionHead title="Life unlocks" sub="Dependency chains. Each step gates the next. Tap the icon to cycle locked, in progress, done." />
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))" }}>
        {data.unlocks.chains.map((ch, ci) => {
          const tint = ci === 0 ? C.yellow : C.mint;
          ...
          return (
            <div key={ch.id} className="rounded-2xl p-4" style={{ ...card, borderRadius: 18 }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1"><InlineEdit value={ch.name} onChange={(v) => patchChain(ch.id, { name: v })} strong /><div style={{ fontSize: 12, color: MUT }}>{done}/{ch.nodes.length} done</div></div>
              </div>
              <div className="flex flex-col">
                {ch.nodes.map((n, i) => (
                  <div key={n.id} className="flex gap-3">
                    ...
                    <div className="flex-1 pb-3" style={{ opacity: n.status === "locked" ? 0.6 : 1, minWidth: 0 }}>
                      <div className="flex items-center gap-1"><div className="flex-1 min-w-0"><InlineEdit value={n.label} onChange={(v) => updateNode(ch.id, n.id, { label: v })} strong /></div><button onClick={() => removeNode(ch.id, n.id)} style={iconBtn} aria-label="Remove"><X size={11} /></button></div>
                      <InlineEdit value={n.sub} onChange={(v) => updateNode(ch.id, n.id, { sub: v })} placeholder="Detail…" dim />
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => addNode(ch.id)} className="flex items-center gap-1 mt-1" style={{ ...iconBtn, color: tint, fontSize: 12 }}><Plus size={12} /> Add step</button>
            </div>
          );
        })}
      </div>
